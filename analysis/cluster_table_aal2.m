function clusters = cluster_table_aal2(clusters, varargin)
...
%     end
    ...
    % ========= aal2 setting
    atlas = which('ROI_MNI_V5.nii');%fullfile(atlas_basedir, 'ROI_MNI_V4.nii');
    MNID = spm_vol(atlas);
    load(which('ROI_MNI_V5_Border.mat')); %load(fullfile(atlas_basedir, 'ROI_MNI_V4_Border.mat'));
    load(which('ROI_MNI_V5_List.mat')); %load(fullfile(atlas_basedir, 'ROI_MNI_V4_List.mat'));   
    ...
    for i = 1:size(cmatx, 1)
        ...
        XYZmm = [x(i); y(i); z(i)];
        Label = gin_det_plabels(XYZmm, MNIY, MNID, ROI,fact);
        clusters(i).shorttitle = Label.Nom;
...
% ========= Suhwan's additional setting

atlas = which('ROI_MNI_V5.nii');
MNID = spm_vol(atlas);
load(which('ROI_MNI_V5_Border.mat')); 
load(which('ROI_MNI_V5_List.mat')); 
MNIY = spm_read_vols(MNID);
fact = 1;
XYZmm = [clusters(1).mm_center(1); clusters(1).mm_center(2); clusters(1).mm_center(3)];
Label = gin_det_plabels(XYZmm, MNIY, MNID, ROI,fact);
region_title = Label.Nom;
% =========

fprintf(fid, '%s\t%3.0f\t%3.0f\t%3.0f\t%3.2f\t%3.0f\t%3.5f\t', ...
    ['s-' region_title], clusters(1).mm_center(1), clusters(1).mm_center(2), clusters(1).mm_center(3), ...
